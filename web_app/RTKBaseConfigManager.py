...
class RTKBaseConfigManager:
    ...
    def get_tracks_settings(self):
        """
            Get settings from the tracks section in an ordered object
            and remove the single quotes.
        """
        ordered_tracks = [{"source_section" : "tracks"}]
        for key in ("trackdir", "file_rotate_time", "archive_rotate"):
            ordered_tracks.append({key : self.config.get('tracks', key).strip("'")})
