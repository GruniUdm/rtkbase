...
$(document).ready(function () {
    ...
    socket.on("services status", function(msg) {
        ...
        // ####################  TRACKS service Switch #########################

        var tracksSwitch = $('#tracks-switch');
        // set the switch to on/off depending of the service status
        if (servicesStatus[7].active === true) {
            tracksSwitch.bootstrapToggle('on', true);
        } else {
            tracksSwitch.bootstrapToggle('off', true);
        }
        if (servicesStatus[7].btn_color) {
            tracksSwitch.bootstrapToggle('setOnStyle', servicesStatus[7].btn_color);
        }
        if (servicesStatus[7].btn_off_color) {
            tracksSwitch.bootstrapToggle('setOffStyle', servicesStatus[7].btn_off_color);
        }
        
        $( "#tracks-switch" ).one("change", function(e) {
            var switchStatus = $(this).prop('checked');
            socket.emit("services switch", {"name" : "tracks", "active" : switchStatus});          
        });          
        })
    })
