...
logging.basicConfig(
    filename='/home/basegnss/rtkbase/web_app/bulk_ndvi.log',
    level=logging.INFO,
    format='%(asctime)s %(message)s',
    filemode='w'
)
logging.info("Starting bulk NDVI recalc for all zones")
...
for i, zid in enumerate(zones):
    ...
    try:
        ...
        logging.error(f"[{i+1}/{len(zones)}] {zid} FAILED: {e}")

logging.info("Bulk NDVI recalc DONE")
